...
Break the SELECTION into EVERY distinct memorizable item it contains, without limiting their number: every content word (noun, verb, adjective, adverb) AND every multi-word unit — phrasal verbs, idioms, set phrases, common collocations and constructions (e.g. "give up", "at first glance", "hard to spot", "not that + adjective"). Be exhaustive — cover the whole selection, not just a few highlights. Return a multi-word unit as its own item IN ADDITION to its useful component words; only when the component words carry no meaning outside the unit, return just the unit. Skip only standalone function words (articles, pronouns, prepositions, conjunctions, auxiliary/modal verbs). Do not skip a word just because it looks common.
